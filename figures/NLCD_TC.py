...
titles_4 = ["(a) no $Q_{AH}$","(b) method 1", "(c) method 2", "(d) method 3"]
...
for i, (folder, ds) in enumerate(datasets.items()):
    ...
    ax.set_xlabel("T$_C$ (K)", fontsize=axis_label_font_size)
    ax.set_ylabel("Diagnosed T$_C$ (K)", fontsize=axis_label_font_size)
    ax.tick_params(axis='both', which='major', labelsize=tick_label_font_size)
...
plt.tight_layout()
plt.show()
fig.savefig("figures/NLCD_combined_TC.png", dpi=300)
