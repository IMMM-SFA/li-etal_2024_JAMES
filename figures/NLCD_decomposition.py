...
method_map = {
    'NLCD/outputs_AH_option4': 'method 1',
    'NLCD/outputs_AH_option3': 'method 2',
    'NLCD/outputs_AH_option2': 'method 3',
    # Add more if needed
}
...
# fig.savefig("figures/NLCD_decomposition.png", dpi=300)


#%%

fig, axes = plt.subplots(1, 3, figsize=(18, 6))  # Setting up a 1x3 grid
axes = axes.ravel()  # Flattening the axes to easily index them
...
fig.savefig("figures/NLCD_decomposition_all.png", dpi=300)
...
axes[1, 1].set_ylim([0, 25])  # Adjusting the y-limit for the summed values
...
# # Define colorbar ticks from -0.5 to 1 with 0.1 intervals
# colorbar_ticks = np.arange(0, 1.01, 0.1)

# for i, folder in enumerate(folder_paths[1:]):
#     # Calculate the difference in T between the methods and the control
#     diff_T = (datasets[folder]['TA_URB].where(ref_ds['UTYPE_URB'] != 0).mean(dim='Time') - ref_ds['TA_URB'].where(ref_ds['UTYPE_URB'] != 0).mean(dim='Time'))

#     ax = axes[i]
#     # Use xarray's built-in plot method
#     im = diff_T.plot(ax=ax, cmap='Reds', vmin=0, vmax=1, add_colorbar=False)
...
# fig.savefig("NLCD_spatial_TA.png", dpi=300)


#%% spatial plot changes in TB_URB normalzied by the amount of anthropogenic heat flux 
...
# colorbar_ticks = np.arange(0, 2.01, 0.2)

# for i, folder in enumerate(folder_paths[1:]):
#     diff_tc_urb_diag_avg_time = datasets[folder]['TB_URB'].where(ref_ds['UTYPE_URB'] != 0).mean(dim='Time') - ref_ds['TB_URB'].where(ref_ds['UTYPE_URB'] != 0).mean(dim='Time')
    
#     ax = axes[i]
#     im = diff_tc_urb_diag_avg_time.plot(ax=ax, cmap='Reds', vmin=0, vmax=2, add_colorbar=False)  # Set add_colorbar=False
#     ax.set_title(titles[i])
...
# plt.tight_layout()
# plt.show()
# fig.savefig("NLCD_spatial_TB.png", dpi=300) 


#%% spatial plot changes in TG_URB normalzied by the amount of anthropogenic heat flux 
...
# colorbar_ticks = np.arange(0, 4.01, 0.4)

# for i, folder in enumerate(folder_paths[1:]):
#     diff_tc_urb_diag_avg_time = datasets[folder]['TG_URB'].where(ref_ds['UTYPE_URB'] != 0).mean(dim='Time') - ref_ds['TG_URB'].where(ref_ds['UTYPE_URB'] != 0).mean(dim='Time')
    
#     ax = axes[i]
#     im = diff_tc_urb_diag_avg_time.plot(ax=ax, cmap='Reds', vmin=0, vmax=4, add_colorbar=False)  # Set add_colorbar=False
#     ax.set_title(titles[i])
...
# plt.tight_layout()
# plt.show()
# fig.savefig("NLCD_spatial_TG.png", dpi=300) 
